...
SELECT     OBJECT_SCHEMA_NAME( pstats.object_id ) AS SchemaName
         ...
         , CASE
               WHEN pf.boundary_value_on_right = 0 THEN
                   c.name + ' > '
                   + CAST(ISNULL(
                              LAG( prv.value ) OVER ( PARTITION BY pstats.object_id ORDER BY pstats.object_id
               ...
               ELSE
                   ...
                   + CAST(ISNULL(
                              LEAD( prv.value ) OVER ( PARTITION BY pstats.object_id ORDER BY pstats.object_id
                                                                                            , pstats.partition_number )
...
LEFT JOIN  sys.partition_range_values AS prv
      ...
      AND pstats.partition_number = ( CASE pf.boundary_value_on_right
                                      ...
                                      END )
--WHERE pstats.object_id = OBJECT_ID('Object')
